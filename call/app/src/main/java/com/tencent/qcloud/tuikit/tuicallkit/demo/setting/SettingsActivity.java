...
public class SettingsActivity extends BaseActivity {
    ...
    private SwitchCompat     mSwitchAddMainView;
    private SwitchCompat     mSwitchUseStreamView;
    private CheckBox         mCheckboxMicrophone;
    private CheckBox         mCheckboxAudioDevice;
    private CheckBox         mCheckboxCamera;
    private CheckBox         mCheckboxSwitchCamera;
    private CheckBox         mCheckboxInviteUser;
    ...
    private void initView() {
        ...
        mSwitchAddMainView = findViewById(R.id.switch_add_main_view);
        mSwitchUseStreamView = findViewById(R.id.switch_use_stream_view);
        mCheckboxMicrophone = findViewById(R.id.checkbox_microphone);
        mCheckboxAudioDevice = findViewById(R.id.checkbox_audio_device);
        mCheckboxCamera = findViewById(R.id.checkbox_camera);
        mCheckboxSwitchCamera = findViewById(R.id.checkbox_switch_camera);
        mCheckboxInviteUser = findViewById(R.id.checkbox_invite_user);
        ...
        mCheckboxMicrophone.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                TUICallKit.createInstance(getApplication()).disableControlButton(Constants.ControlButton.Microphone);
            } else {
                GlobalState.Companion.getInstance().getDisableControlButtonSet().remove(Constants.ControlButton.Microphone);
            }
        });
        mCheckboxAudioDevice.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                TUICallKit.createInstance(getApplication()).disableControlButton(Constants.ControlButton.AudioPlaybackDevice);
            } else {
                GlobalState.Companion.getInstance().getDisableControlButtonSet().remove(Constants.ControlButton.AudioPlaybackDevice);
            }
        });
        mCheckboxCamera.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                TUICallKit.createInstance(getApplication()).disableControlButton(Constants.ControlButton.Camera);
            } else {
                GlobalState.Companion.getInstance().getDisableControlButtonSet().remove(Constants.ControlButton.Camera);
            }
        });
        mCheckboxSwitchCamera.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                TUICallKit.createInstance(getApplication()).disableControlButton(Constants.ControlButton.SwitchCamera);
            } else {
                GlobalState.Companion.getInstance().getDisableControlButtonSet().remove(Constants.ControlButton.SwitchCamera);
            }
        });
        mCheckboxInviteUser.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                TUICallKit.createInstance(getApplication()).disableControlButton(Constants.ControlButton.InviteUser);
            } else {
                GlobalState.Companion.getInstance().getDisableControlButtonSet().remove(Constants.ControlButton.InviteUser);
            }
        });
        mSwitchAddMainView.setOnCheckedChangeListener((buttonView, isChecked) -> {
            CallAdapter adapter = null;
            if (isChecked) {
                mSwitchUseStreamView.setChecked(false);
                adapter = SettingsConfig.customUIAdapter.getMainViewAdapter();
            }
            TUICallKit.createInstance(getApplication()).setAdapter(adapter);
        });

        mSwitchUseStreamView.setOnCheckedChangeListener((buttonView, isChecked) -> {
            CallAdapter adapter = null;
            if (isChecked) {
                mSwitchAddMainView.setChecked(false);
                adapter = SettingsConfig.customUIAdapter.getStreamViewAdapter();
            }
            TUICallKit.createInstance(getApplication()).setAdapter(adapter);
        });
    ...
    private void initData() {
        ...
        Set<Constants.ControlButton> buttonSet = GlobalState.Companion.getInstance().getDisableControlButtonSet();
        mCheckboxMicrophone.setChecked(buttonSet.contains(Constants.ControlButton.Microphone));
        mCheckboxAudioDevice.setChecked(buttonSet.contains(Constants.ControlButton.AudioPlaybackDevice));
        mCheckboxCamera.setChecked(buttonSet.contains(Constants.ControlButton.Camera));
        mCheckboxSwitchCamera.setChecked(buttonSet.contains(Constants.ControlButton.SwitchCamera));
        mCheckboxInviteUser.setChecked(buttonSet.contains(Constants.ControlButton.InviteUser));
    }
